...
import { updateParcelExchangeRate } from "@/actions/parcels";
...
export default function ExchangeRateForm({
  ...
  const subtotalCny = totalUsd * rate + localDeliveryCny;
  const recalculatedCny = subtotalCny - subtotalCny * (discountPercent / 100);

  const handleSubmit = (formData: FormData) => {
    startTransition(async () => {
      const result = await updateParcelExchangeRate(orderId, formData);
